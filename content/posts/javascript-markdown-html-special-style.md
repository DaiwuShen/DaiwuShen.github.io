---
title: "「Javascript」解析markdown为html——特殊样式篇"
date: 2023-03-21T00:00:00+08:00
lastmod: 2023-03-21T00:00:00+08:00
tags: ["Javascript", "解析", "markdown", "html"]
categories: ["计算机"]
image: "https://cdn.staticaly.com/gh/DaiwuShen/daiwuImageBed@main/blog/anime-anime-girls-science-fiction-fantasy-girl-Fantasy-Architecture-2210141-wallhere.com.jpg"
description: "在记录笔记或撰写文章时，时常需要突出重点。本文整理 markdown 中加粗、斜体、高亮三种特殊样式如何解析为对应的 html 标签。"
draft: false
---

## 1 前言

在记录笔记或撰写文章时，时常需要突出重点，让自己和读者关注到重要内容。在 markdown 和 html 中，常见的着重样式有 **加粗** ，<mark>高亮</mark> ， *斜体* 。在我的个人博客中，为了更方便更完整地转换格式，在转换脚本中也加入了这方面的转换。
...
在 markdown 中加粗样式虽然有两种标记，但都是 <mark>前后各两个"*"或"_"进行包裹</mark> ，可以精确地对两符号之间的内容渲染加粗样式。对应于 html 则只需用 `<strong>` 标签包裹即可。
...
markdown 语法很简单，<mark>首尾各一个"*"或"_"时表示斜体</mark> ，两个时表示加粗。对应于 html 的 `<em>` 标签。
...
代码中，运用了 <mark>match</mark> 函数进行正则表达式匹配出特定格式的文本，也运用该函数匹配出样式标记包裹的文本。[正则表达式测试工具](/tools/regex-tester/)在这里。
